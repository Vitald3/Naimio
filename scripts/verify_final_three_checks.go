...
func (c *Checker) RunCheck2Migration053To054() {
	...
	testDSN := "postgres://freelance:freelance_dev_secret@localhost:5432/" + testDBName + "?sslmode=disable"
	...
	_, err = testDB.Exec(`INSERT INTO users (id, email, email_normalized, password_hash, display_name) VALUES
		($1, 'test-mig@example.test', 'test-mig@example.test', 'hash', 'Test Migration')`, testOwnerID)
	if err != nil {
	...
	_, _ = testDB.Exec(`INSERT INTO feature_flags (key, enabled, config) VALUES ('file_storage', true, $1::jsonb)`, ffJSON)
	_, _ = testDB.Exec(`INSERT INTO storage_credentials (provider, encrypted_config) VALUES ('s3', $1)`, encSecret)
	...
	_, err = testDB.Exec(`INSERT INTO media_objects (id, owner_user_id, purpose, storage_provider, object_key, bucket, original_filename, mime_type, size_bytes, scan_status, uploaded_at)
		VALUES ($1, $2, 'PORTFOLIO', 'local', $3, 'local-media', 'local-pre-mig.png', 'image/png', $4, 'CLEAN', now())`,
		localID, testOwnerID, localKey, len(localContent))
	if err != nil {
	...
	_, err = testDB.Exec(`INSERT INTO media_objects (id, owner_user_id, purpose, storage_provider, object_key, bucket, original_filename, mime_type, size_bytes, scan_status, uploaded_at)
		VALUES ($1, $2, 'PORTFOLIO', 's3', $3, 'files', 's3-pre-mig.png', 'image/png', $4, 'CLEAN', now())`,
		s3ID, testOwnerID, s3Key, len(s3Content))
	if err != nil {
	...
	var locProv, locBackendProv string
	var locBackendID, locBackendIDRef sql.NullString
	err = testDB.QueryRow(`
		SELECT m.storage_provider, m.storage_backend_id::text, sb.id::text, sb.provider
		FROM media_objects m
		LEFT JOIN storage_backends sb ON sb.id = m.storage_backend_id
		WHERE m.id = $1`, localID).Scan(&locProv, &locBackendID, &locBackendIDRef, &locBackendProv)
	if err != nil || locProv != "local" || locBackendProv != "local" || !locBackendID.Valid {
		log.Fatalf("Post-migration local object validation failed: locProv=%s, locBackendProv=%s, err=%v", locProv, locBackendProv, err)
	}
	log.Printf("Post-migration Local object: provider=%s, backend_id=%s, backend_provider=%s (PASS)", locProv, locBackendID.String, locBackendProv)

	var s3Prov, s3BackendProv, s3Endpoint, s3AccessKey string
	var s3BackendID sql.NullString
	err = testDB.QueryRow(`
		SELECT m.storage_provider, m.storage_backend_id::text, sb.provider, sb.endpoint, sb.access_key_id
		FROM media_objects m
		LEFT JOIN storage_backends sb ON sb.id = m.storage_backend_id
		WHERE m.id = $1`, s3ID).Scan(&s3Prov, &s3BackendID, &s3BackendProv, &s3Endpoint, &s3AccessKey)
	if err != nil || s3Prov != "s3" || s3BackendProv != "s3" || s3Endpoint != "http://minio-a:9000" || s3AccessKey != "endpoint-a-user" {
		log.Fatalf("Post-migration S3 object validation failed: s3Prov=%s, s3BackendProv=%s, endpoint=%s, key=%s, err=%v", s3Prov, s3BackendProv, s3Endpoint, s3AccessKey, err)
	}
	log.Printf("Post-migration S3 object: provider=%s, backend_id=%s, backend_provider=%s, endpoint=%s, access_key=%s (PASS)", s3Prov, s3BackendID.String, s3BackendProv, s3Endpoint, s3AccessKey)

	// 5. Verify physical read through StorageManager
	testManager, err := media.NewStorageManager(testDB, "naimio-default-storage-key-change-in-prod", &media.DiskStorage{RootDir: "/var/lib/naimio-media"}, objectstorage.S3Config{})
	if err != nil {
		log.Fatalf("NewStorageManager failed: %v", err)
	}
	if err := testManager.LoadFromDB(context.Background()); err != nil {
		log.Fatalf("LoadFromDB failed: %v", err)
	}

	testService := media.Service{
		Repository: media.PostgresRepository{DB: testDB},
		Storage:    &media.DiskStorage{RootDir: "/var/lib/naimio-media"},
		Resolver:   testManager,
	}

	// Read local object
	viewLoc, err := testService.Get(context.Background(), testOwnerID, localID)
	if err != nil || viewLoc.DownloadURL == "" {
		log.Fatalf("Get local object after migration failed: %v", err)
	}
	...
	// Read S3 object using resolved backend
	viewS3, err := testService.Get(context.Background(), testOwnerID, s3ID)
	if err != nil || viewS3.DownloadURL == "" {
		log.Fatalf("Get S3 object after migration failed: %v", err)
	}
	...
	// 6. Delete both objects
	if err := testService.Delete(context.Background(), testOwnerID, localID); err != nil {
		log.Fatalf("Delete local object after migration failed: %v", err)
	}
	if _, err := exec.Command("docker", "exec", "freelance-api-1", "ls", "/var/lib/naimio-media/"+localKey).CombinedOutput(); err == nil {
		log.Fatalf("Local file was not physically deleted after migration!")
	}

	if err := testService.Delete(context.Background(), testOwnerID, s3ID); err != nil {
		log.Fatalf("Delete S3 object after migration failed: %v", err)
	}
	if _, err := exec.Command("docker", "exec", "freelance-minio-a-1", "mc", "stat", "minio-a/files/"+s3Key).CombinedOutput(); err == nil {
		log.Fatalf("S3 file was not physically deleted from MinIO A after migration!")
	}
...
func (c *Checker) RunCheck3ActiveBackendInvariant() {
	...
	_, _ = c.db.Exec(`UPDATE storage_backends SET is_active = false`)
	_, _ = c.db.Exec(`
	...
	var activeCount int
	var activeProvider, activeEndpoint, activeBucket string
	err = c.db.QueryRow(`
		SELECT count(*), COALESCE(max(provider), ''), COALESCE(max(endpoint), ''), COALESCE(max(bucket), '')
		FROM storage_backends WHERE is_active = true`).Scan(&activeCount, &activeProvider, &activeEndpoint, &activeBucket)
	if err != nil || activeCount != 1 {
		log.Fatalf("INVARIANT VIOLATION: Expected exactly 1 active backend, got %d!", activeCount)
	}
	log.Printf("Active backends in DB after 20 concurrent updates: %d (provider=%s, endpoint=%s, bucket=%s) (PASS)",
		activeCount, activeProvider, activeEndpoint, activeBucket)
