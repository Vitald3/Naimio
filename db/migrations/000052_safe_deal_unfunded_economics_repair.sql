...
-- Repair only unfunded deals whose economic snapshot can still be changed safely.
-- This deliberately re-snapshots the currently active payer rule because these
-- rows have not been funded yet. Real-provider pending/processed operations are
-- excluded. Debug sandbox PENDING records are local-only and can be kept in sync.
WITH active_rule AS (
  SELECT version,
         platform_fee_payer_mode,
         platform_customer_share_basis_points,
         provider_fee_payer_mode,
         provider_customer_share_basis_points
  FROM safe_deal_fee_rules
  WHERE enabled AND effective_from<=now()
  ORDER BY effective_from DESC
  LIMIT 1
), eligible AS (
  SELECT d.id,
         COALESCE(d.work_amount_kopecks, d.gross_amount_kopecks) AS work,
         d.platform_fee_kopecks AS platform_fee,
         d.provider_fee_kopecks AS provider_fee,
         r.version AS fee_rule_version,
         r.platform_fee_payer_mode,
         r.platform_customer_share_basis_points,
         r.provider_fee_payer_mode,
         r.provider_customer_share_basis_points,
         d.customer_discount_kopecks,
         d.freelancer_bonus_kopecks
  FROM safe_deals d
  CROSS JOIN active_rule r
  WHERE d.status='AWAITING_FUNDING'
    AND NOT EXISTS (
      SELECT 1 FROM payment_records pr
      WHERE pr.deal_id=d.id
        AND NOT (pr.provider='sandbox' AND pr.provider_status='PENDING')
    )
), allocation AS (
  SELECT e.*,
    CASE e.platform_fee_payer_mode
      WHEN 'CUSTOMER' THEN e.platform_fee
      WHEN 'SPLIT' THEN (e.platform_fee*e.platform_customer_share_basis_points)/10000
      ELSE 0 END AS platform_customer,
    CASE e.platform_fee_payer_mode
      WHEN 'FREELANCER' THEN e.platform_fee
      WHEN 'SPLIT' THEN e.platform_fee-(e.platform_fee*e.platform_customer_share_basis_points)/10000
      ELSE 0 END AS platform_freelancer,
    CASE WHEN e.platform_fee_payer_mode='PLATFORM' THEN e.platform_fee ELSE 0 END AS platform_platform,
    CASE e.provider_fee_payer_mode
      WHEN 'CUSTOMER' THEN e.provider_fee
      WHEN 'SPLIT' THEN (e.provider_fee*e.provider_customer_share_basis_points)/10000
      ELSE 0 END AS provider_customer,
    CASE e.provider_fee_payer_mode
      WHEN 'FREELANCER' THEN e.provider_fee
      WHEN 'SPLIT' THEN e.provider_fee-(e.provider_fee*e.provider_customer_share_basis_points)/10000
      ELSE 0 END AS provider_freelancer,
    CASE WHEN e.provider_fee_payer_mode='PLATFORM' THEN e.provider_fee ELSE 0 END AS provider_platform
  FROM eligible e
), calculated AS (
  SELECT a.*,
    a.work+a.platform_customer+a.provider_customer-a.customer_discount_kopecks AS customer_total,
    a.work-a.platform_freelancer-a.provider_freelancer+a.freelancer_bonus_kopecks AS freelancer_payout
  FROM allocation a
)
UPDATE safe_deals d
SET work_amount_kopecks=c.work,
    gross_amount_kopecks=c.customer_total,
    freelancer_amount_kopecks=c.freelancer_payout,
    platform_fee_customer_kopecks=c.platform_customer,
    platform_fee_freelancer_kopecks=c.platform_freelancer,
    platform_fee_platform_kopecks=c.platform_platform,
    provider_fee_customer_kopecks=c.provider_customer,
    provider_fee_freelancer_kopecks=c.provider_freelancer,
    provider_fee_platform_kopecks=c.provider_platform,
    platform_fee_payer_mode=c.platform_fee_payer_mode,
    platform_customer_share_basis_points=c.platform_customer_share_basis_points,
    provider_fee_payer_mode=c.provider_fee_payer_mode,
    provider_customer_share_basis_points=c.provider_customer_share_basis_points,
    fee_rule_version=c.fee_rule_version,
    platform_provider_cost_kopecks=c.provider_platform,
    platform_subsidy_kopecks=c.platform_platform+c.provider_platform+c.customer_discount_kopecks+c.freelancer_bonus_kopecks,
    platform_net_revenue_kopecks=c.customer_total-c.provider_fee-c.freelancer_payout,
    updated_at=now()
FROM calculated c
WHERE d.id=c.id
  AND c.customer_total>0
  AND c.freelancer_payout>0;

UPDATE payment_records pr
SET amount_kopecks=d.gross_amount_kopecks,
    updated_at=now()
FROM safe_deals d
WHERE pr.deal_id=d.id
  AND pr.provider='sandbox'
  AND pr.provider_status='PENDING'
  AND d.status='AWAITING_FUNDING'
  AND pr.amount_kopecks<>d.gross_amount_kopecks;
